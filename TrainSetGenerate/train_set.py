...
python gen_printed_char.py --out_dir ./dataset \
    ...
    print(verified_font_paths)
    font2image = []
    for i in range (margin+1):
        font2image.append(Font2Image(width, height, need_crop, margin+1))

    for (char, value) in lang_chars.items():  # 外层循环是字
        print("num " + str(value) + " is " + str(char))
        image_list = []
        #print (char)
        #char_dir = os.path.join(images_dir, "%0.5d" % value)
        flag = 0
        for j, verified_font_path in enumerate(verified_font_paths):    # 内层循环是字体 
            for z in range(0,margin+1,2):
                if(flag == 0):
                    print(str(char) + " margin range is 0-" + str(margin))
                    flag = 1
                #print("now margin is : " + str(z))
                if rotate == 0:
                    image = font2image[z].do(verified_font_path, char)
                    image_list.append(image)
                else:
                    for k in all_rotate_angles:	
                        image = font2image[z].do(verified_font_path, char, rotate=k)
                        image_list.append(image)
        ...
        for i in range(len(image_list)):
            ...
            count += 1
    e_time = time.time()
    print("sum time is : " + str(e_time - s_time))
