...
def locate_picture2(src, limit_color, limit_number, limit, up_limit, down_limit):
    #show_img(src)
    ...
    start_location = split_list[record][0]
    end_location = split_list[record][1]
...
def horizontal_split_chinese4(src,store, show,horizontal_id):
    limit_color = 250
    limit_number = 10
    scope = 4
    #腐蚀图像
            ...
            if width > min_width:
                if width > max_width:
                    while width > max_width:
                        column_image = src[0: src.shape[0]-4, char_index: char_index + standard_width - 1]
                        column_image = cv2.resize(column_image, (0, 0), fx=2, fy=2, interpolation=cv2.INTER_AREA)
                        column_list.append(column_image)
                        char_index = char_index + standard_width
                        width = blank_index - char_index
                    column_image = src[0: src.shape[0]-4, char_index: blank_index - 1]
                    column_image = cv2.resize(column_image, (0, 0), fx=2, fy=2, interpolation=cv2.INTER_AREA)
                    column_list.append(column_image)
                else:
                    column_image = src[0: src.shape[0]-4, char_index: blank_index - 1]
...
#show:是否显示
#horizontal_id:生成图片编号
def horizontal_split_char4(src, store, show, horizontal_id):
    limit_color = 250
    limit_number = 15
    #show_img(src)
    color_number = get_pixel_matrix(src, 0) / limit_color
    ...
    start_location = int(src.shape[1] * 0.4)   # 记录的起始位置
    standard_width = int(src.shape[0] * 0.44)  # 标准的字符宽度
    min_width = int(src.shape[0] * 0.12)        #最小宽度
    ...
    for index in range(start_location, src.shape[1]):
        ...
        # 进入空白区
            ...
            if width > min_width:
                if width > max_width:
                    while width > max_width:
                        column_image = src[0: src.shape[0] - 14, char_index: char_index + standard_width]
                        column_image = cv2.resize(column_image, (0, 0), fx=2.5, fy=2.8, interpolation=cv2.INTER_AREA)
                        column_list.append(column_image)
                        char_index = char_index + standard_width
                        width = blank_index - char_index
                    column_image = src[0: src.shape[0] - 14, char_index: blank_index]
                    column_image = cv2.resize(column_image, (0, 0), fx=2.5, fy=2.8, interpolation=cv2.INTER_AREA)
                    column_list.append(column_image)
                else:

                    column_image = src[0: src.shape[0] - 14, char_index - 1: blank_index + 2]
                    column_image = cv2.resize(column_image, (0, 0), fx=2.5, fy=2.8, interpolation=cv2.INTER_AREA)
                    column_list.append(column_image)
