...
tf.app.flags.DEFINE_integer('charset_size', char_size, "Choose the first `charset_size` characters only.")
tf.app.flags.DEFINE_integer('image_size', 64, "Needs to provide same value as in training.")
tf.app.flags.DEFINE_boolean('gray', True, "whether to change the rbg to gray")
tf.app.flags.DEFINE_integer('max_steps', 35100, 'the max training steps ')
...
def train():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=True)) as sess:
        ...
        graph = build_graph(top_k=3)  # 训练时top k = 1
...
def main(_):
    ...
    if FLAGS.mode == "train":
        ...
        name_list = get_file_list(test_file_content)
